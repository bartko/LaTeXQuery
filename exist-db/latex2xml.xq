    ...
    return 
        typeswitch($node)
            case text() return 
                ...
                let $node  := local:parse-xml(concat('<x>',$node,'</x>'))
                   return $node/x/node()
               )
            default return $node 
}
...
for $node in $node/node()
   ...
   let $text :=
     try {  
        local:parse-xml(concat('<text>',$node,'</text>'))
     } catch * {
...
let $latex := util:binary-doc($latexpath)
let $latex := util:binary-to-string($latex)
...
let $xml := local:latex2xml('/db/latexfile.tex')
